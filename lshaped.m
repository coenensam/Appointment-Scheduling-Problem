...
function [w_v,x_v,theta_v,st_dev,CI] = lshaped(T,h2,q,n,k,p,x,d) 
h = h2;
h(end,:) = h(end,:) - d;
% the initial x is given
c_i = q(2,1);
c_w = q(1,1);
c_l = q(3,1);
Dual_multip = dual_recursion(h2,c_i,c_w,c_l,n,k,x,d);

b=-p*(sum(Dual_multip.*h))';
A_temp = p*Dual_multip'*T;
A = [-A_temp,-1];
lbx = zeros(n,1);
condition = 0;

while (condition == 0)
Master_c = [zeros(1,n-1),1];
Master_sol = linprog(Master_c,A,b,[],[],lbx,[]);

theta_v = Master_sol(n,1); % theta from the master is the lower bound
...
w_v = p*(sum(Dual_c.*Dual_multip))'; 
 ...
 if((w_v-theta_v)/theta_v>0.00001)   % check whether upper bound is greater than lower bound
        A_temp = p*Dual_multip'*T; % if so, add optimality cut
        A = [A;-A_temp,-1];
        b = [b;-p*(sum(Dual_multip.*h))'];
 else
     condition = 1;
