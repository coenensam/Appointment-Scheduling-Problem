%% L-shaped multi cut for dynamic scheduling
function [w_v,x,theta_v] = dynamic_lshaped_multi(h2,q,n,n_u,k,p,p_u,x,d) % single cut
cuts = cell(2,n_u+1);
w_v = zeros(1,n_u+1);
A = [];
b = [];
...
lbx = zeros(n+n_u-1+k,1);
Master_c = [zeros(1,n+n_u-1),p];  
Master_sol = linprog(Master_c,A,b,[],[],lbx,[]);
 
theta_v = p*Master_sol(n+n_u:end,1);
